...
public class CombinationSum {
    ...
    public static void main(String[] args){
        ...
        s.add(a);s.add(b);
        test(new int[]{2,3,6,7}, 7, s);

    }

    public static void test(int [] num, int target, List<List<Integer>> solution){
        List<List<Integer>> r=combinationSum(num, target) ;
    ...
    public static List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        if(n>0){
            combination(candidates, target, ret, new ArrayList<Integer>(), 0, n);
        }
        return ret;
    }

    public static void combination(int[] candidates, int target, List<List<Integer>> ret, List<Integer> cur, int start, int end){
        if(target==0) {
        ...
        for(int i=start; i<end;i++){
            ...
            if(rest>=0){
                ...
                combination(candidates,rest, ret, cur, i, end);
